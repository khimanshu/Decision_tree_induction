//CS - 641 
//Hk0014 and Ae0027
//Aditya Krishna Edara and Himanshu Koyalkar
//Spring - 2017
...
ofstream myfile;                                    //for writing to files that are being created 
ofstream appfile;
ofstream accfile;
void setIndex(set<string> attrList)			//creating index for the records in the input file
{
	int count =0;
...
vector< vector<string> > getData(vector< vector<string> > data, string attr, string attrVal) //getting data into vector
{
...
string maximal(set<string> targetVals)   //selecting maximum attribute
{
...
string  choose_attribute(vector< vector<string> > data, set<string> attrList, string targetAttr) //selecting attribute for splitting
{
...
tree*  construct_tree(vector< vector<string> > data,  set<string> attrList, string targetAttr) //creating tree
{
...
string  traverseTree(tree *T, vector<string> row) //traversing the tree
{
	bool leaf =1;
...
void printLeaves(tree *T,int depth) //printing the leaf nodes
{
	if(T==NULL)
		return;
...
void printVector( vector<string>  newData) //printing the vectors
{
...
void printVector_File( vector<string>  newData) //printing data from file
{
	 for(vector<string> ::iterator it = newData.begin(); it != newData.end(); ++it)
        {
...
int main(int ac,char** argv)		//main function
{
